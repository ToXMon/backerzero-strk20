...
6. **Real-proof environment (new, blocking):** Which storage-proof-capable path will be used for real-proof verification — a self-hosted Pathfinder `v0.22.7` or an RPC endpoint exposing `starknet_getStorageProof` — and on which network, with which disposable funded account and which privacy-pool/proving-service deployment? `starknet-devnet` cannot serve the prover (JSON-RPC error 42), so this gates every remaining privacy claim. See `docs/TECHNICAL_VERIFICATION.md` §9.
7. **Refund authorization:** Can a plain receipt preimage be replayed or stolen from public claim calldata before settlement? Must the MVP add destination binding or one-time signing, or explicitly ship the bearer capability as experimental?
